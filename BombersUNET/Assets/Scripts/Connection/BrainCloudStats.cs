...
namespace BrainCloudUNETExample.Connection
{
    public class BrainCloudStats : MonoBehaviour
    {
        ...
        {
            m_leaderboardReady = false;
            BombersNetworkManager._BC.Client.SocialLeaderboardService.GetGlobalLeaderboardPage(
				aLeaderboardID, 
				BrainCloud.BrainCloudSocialLeaderboard.SortOrder.HIGH_TO_LOW, 
				aIndex, 
				aSecondIndex, 
				LeaderboardSuccess_Callback, 
				LeaderboardFailure_Callback, 
				null);
        }
        ...
        {
            BombersNetworkManager._BC.Client.GlobalAppService.ReadProperties(PropertiesSuccess_Callback, PropertiesFailure_Callback, null);
        }

        public void ReadStatistics()
        {
            // Ask brainCloud for statistics
            BombersNetworkManager._BC.Client.PlayerStatisticsService.ReadAllUserStats(StatsSuccess_Callback, StatsFailure_Callback, null);
            BombersNetworkManager._BC.Client.PlayerStateService.ReadUserState(StateSuccess_Callback, StateFailure_Callback, null);
            BombersNetworkManager._BC.Client.GamificationService.ReadXpLevelsMetaData(LevelsSuccess_Callback, LevelsFailure_Callback, null);
            BombersNetworkManager._BC.Client.GamificationService.ReadAchievements(true, AchievementSuccess_Callback, AchievementFailure_Callback, null);
        }
                ...
                for (int i = 0; i < m_achievements.Count; i++)
                {
                    if (m_achievements[i].m_id == "2")
                    {
                        if (!m_achievements[i].m_achieved)
                        {
                            ...
                            BombersNetworkManager._BC.Client.GamificationService.AwardAchievements(m_achievements[i].m_id, null, null, null);
            ...
            if (m_statCarriersDestroyed >= 10)
            {
                for (int i = 0; i < m_achievements.Count; i++)
                {
                    if (m_achievements[i].m_id == "1")
                    {
                        ...
                        {
                            m_achievements[i].m_achieved = true;
                            BombersNetworkManager._BC.Client.GamificationService.AwardAchievements(m_achievements[i].m_id, AwardSuccess_Callback, AwardFailure_Callback, null);
                            GameObject.Find("DialogDisplay").GetComponent<DialogDisplay>().DisplayAchievement(int.Parse(m_achievements[i].m_id), m_achievements[i].m_name, m_achievements[i].m_description);
                        }
                        break;
                    }
            ...
            for (int i = 0; i < m_achievements.Count; i++)
            {
                if (m_achievements[i].m_id == "0")
                {
                    if (!m_achievements[i].m_achieved)
                    {
                        ...
                        BombersNetworkManager._BC.Client.GamificationService.AwardAchievements(achArray, null, null, null);
                        GameObject.Find("DialogDisplay").GetComponent<DialogDisplay>().DisplayAchievement(int.Parse(m_achievements[i].m_id), m_achievements[i].m_name, m_achievements[i].m_description);
                    }
                    break;
                }
        ...
        public void SubmitLeaderboardData(int aKills, int aBombHits, int aDeaths)
        {
            ...
            BombersNetworkManager._BC.Client.SocialLeaderboardService.PostScoreToLeaderboard("KDR", killScore, "{\"rank\":\"" + m_playerLevelTitles[m_playerLevel - 1] + "\", \"level\":\"" + m_playerLevel + "\"}");
            BombersNetworkManager._BC.Client.SocialLeaderboardService.PostScoreToLeaderboard("BDR", bombScore, "{\"rank\":\"" + m_playerLevelTitles[m_playerLevel - 1] + "\", \"level\":\"" + m_playerLevel + "\"}");
            ReadStatistics();
        }
                                                ...
                                                int carriersDestroyed = 0, int bombsHit = 0)
        {
            ...
            // Send to the cloud
            BombersNetworkManager._BC.Client.PlayerStatisticsService.IncrementUserStats(
        ...
        public void IncrementExperienceToBrainCloud(int aExperience)
        {
            ...
            BombersNetworkManager._BC.Client.PlayerStatisticsService.IncrementExperiencePoints(aExperience, StateSuccess_Callback, StateFailure_Callback, null);
        }
