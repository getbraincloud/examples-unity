        ...
        void OnCollisionEnter(Collision aCollision)
        {
            if (m_bulletInfo.m_isMaster)
            {
                if (aCollision.gameObject.GetComponent<PlaneController>() != null)
                    GameObject.Find("GameManager").GetComponent<GameManager>().CmdBulletHitPlayer(m_bulletInfo, aCollision);
                else
                    GameObject.Find("GameManager").GetComponent<GameManager>().CmdDeleteBullet(m_bulletInfo);
            }
        }
