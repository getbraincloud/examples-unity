...
public class UIParticleSystem : MaskableGraphic
{
    ...
    // regular way to anim
    public bool SMOOTH_ANIM = true;
    private float _durationOfAnim;
    ...
    protected override void OnPopulateMesh(VertexHelper vh)
    {
        ...
        if (!SMOOTH_ANIM)
            _durationOfAnim -= Time.deltaTime;

        for (int i = 0; i < count; ++i)
        {
            ParticleSystem.Particle particle = _particles[i];

            if (_durationOfAnim <= 0.0f && !SMOOTH_ANIM)
            {
                _durationOfAnim = particle.startLifetime;
            }
            ...
            if (particle.remainingLifetime + 0.15f >= particle.startLifetime || SMOOTH_ANIM)
            {
