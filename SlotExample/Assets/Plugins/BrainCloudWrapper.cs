...
public class BrainCloudWrapper : MonoBehaviour
{
    ...
    public static void Initialize(string url, string secretKey, string gameId, string gameVersion)
    {
        ...
        bcw.m_lastUrl = url;
        bcw.m_lastSecretKey = secretKey;
        bcw.m_lastGameId = gameId;
        bcw.m_lastGameVersion = gameVersion;
        bcw.m_client.Initialize(url, secretKey, gameId, gameVersion);
    }
	...
	public void AuthenticateEmailPassword(string username, string password, bool in_bCreateNew, SuccessCallback in_successCb, FailureCallback in_failureCb)
	{
		m_client.AuthenticationService.AuthenticateEmailPassword(username, password, in_bCreateNew, in_successCb, in_failureCb);
	}
