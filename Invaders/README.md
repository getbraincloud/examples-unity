...
Send the recording to brainCloud for later use
Retrieve the recording when it is needed
Act out the recording during the game to create a teammate
...
We will first grab the player’s previous score from the leaderboard. If the recording’s score is less than the previous best, we break out of the function. This will save unnecessary API calls.
 ...
 - *__Note:__ In this case we compare previousScore to newScore using `>` instead of `>=`. This will favour the most recent recording in the case of a tie. Consider carefully how you filter which recordings you save.*
