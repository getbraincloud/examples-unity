...
{
    public class FishyBrainCloud : Transport
    {
        ...
        //private ClientSocket _clientHostSocket;
        ...
        public override void Initialize(NetworkManager networkManager, int transportIndex)
        {
            ...
            //_clientHostSocket = new Client.ClientSocket();
            //_serverSocket = new Server.ServerSocket();
            _connectedClients = new List<int>();
            CreateChannelData();

            //_client.Initialize(this);
            //_clientHostSocket.Initialize(this);
        ...
        {
            /*
            if(_clientHostSocket != null)
                _clientHostSocket.CheckSetStarted();
            */
        }

        private void Awake()
        {
            StartCoroutine(GetBCManager());
        }

        private IEnumerator GetBCManager()
        {
            while(_brainCloud == null)
            {
                _brainCloud = BCManager.Instance;
                yield return null;
            }

            Debug.Log("Found BCManager");
        }
        #endregion
        ...
        public bool IsNetworkAccessAvailable() => _brainCloud != null && _brainCloud.bc.Client.IsAuthenticated();
        #endregion
        ...
        {
            if (_brainCloud.bc.RelayService.IsConnected())
            {
        ...
        public override void SendToServer(byte channelId, ArraySegment<byte> segment)
        {
            if (GetConnectionState(_isServer) != LocalConnectionState.Started)
                return;
            if (!_brainCloud.bc.Client.IsAuthenticated())
                return;
        ...
        void Send(int recipientId, ArraySegment<byte> segment, byte channelId)
        {
            ...
            _brainCloud.bc.RelayService.Send(
                in_data: data,
        ...
        public void SendToAll(ArraySegment<byte> segment, byte channelId)
        {
            ...
            BCManager.Instance.bc.RelayService.SendToAll(
        ...
        public override bool StartConnection(bool server)
        {
            ...
            _brainCloud.bc.RelayService.RegisterRelayCallback(OnRelayCallback);
            _brainCloud.bc.RelayService.RegisterSystemCallback(OnRelaySystemCallback);

            RelayConnectOptions connectionOptions = new RelayConnectOptions(true,
                _brainCloud.RoomAddress,
                _brainCloud.RoomPort,
                _brainCloud.RelayPasscode,
                _brainCloud.CurrentLobbyId);

            SuccessCallback successCallback = (responseData, cbObject) =>
            {
                bool isRelayConnected = _brainCloud.bc.RelayService.IsConnected();
                if (_isServer)
                {
            ...
            _brainCloud.bc.RelayService.Connect(RelayConnectionType.UDP, connectionOptions, successCallback, failureCallback);
        ...
        private void OnRelaySystemCallback(string json)
        {
            ...
            switch (systemEvent.op)
            {
                case "CONNECT":
                    ...
                    string connectedProfileId = BCManager.Instance.bc.RelayService.GetProfileIdForNetId((short)int.Parse(connectEvent.netId));
                    hostCxId = connectEvent.ownerCxId;

                    hostId = BCManager.Instance.bc.RelayService.GetNetIdForCxId(hostCxId);
                ...
                case "DISCONNECT":
                    ...
                    int disconnectedNetId = _brainCloud.bc.RelayService.GetNetIdForCxId(disconnectEvent.cxId);
                ...
                case "MIGRATE_OWNER":
                    ...
                    int newHostNetID = _brainCloud.bc.RelayService.GetNetIdForCxId(migrateEvent.cxId);
                    hostId = newHostNetID;
        ...
        {
            _brainCloud.bc.RelayService.DeregisterRelayCallback();
            _brainCloud.bc.RelayService.Disconnect();
