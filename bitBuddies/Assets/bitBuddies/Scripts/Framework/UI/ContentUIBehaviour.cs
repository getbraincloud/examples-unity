...
public abstract class ContentUIBehaviour : MonoBehaviour
{
    ...
    /// <summary>
    /// A quick method for ContentUIBehaviours to create a <see cref="SuccessCallback"/> with <see cref="BrainCloudManager.HandleSuccess(string, Action)"/>.
    /// </summary>
    protected SuccessCallback OnSuccess(string logMessage, Action onSuccess) =>
        BrainCloudManager.HandleSuccess(logMessage, onSuccess);

    /// <summary>
    /// A quick method for ContentUIBehaviours to create a <see cref="SuccessCallback"/> with <see cref="BrainCloudManager.HandleSuccess(string, Action{string})"/>.
    /// </summary>
    protected SuccessCallback OnSuccess(string logMessage, Action<string> onSuccessS) =>
        BrainCloudManager.HandleSuccess(logMessage, onSuccessS);

    /// <summary>
    /// A quick method for ContentUIBehaviours to create a <see cref="SuccessCallback"/> with <see cref="BrainCloudManager.HandleSuccess(string, Action{string, object})"/>.
    /// </summary>
    protected SuccessCallback OnSuccess(string logMessage, Action<string, object> onSuccessSO) =>
        BrainCloudManager.HandleSuccess(logMessage, onSuccessSO);

    /// <summary>
    /// A quick method for ContentUIBehaviours to create a <see cref="FailureCallback"/> with <see cref="BrainCloudManager.HandleFailure(string, Action)"/>.
    /// </summary>
    protected FailureCallback OnFailure(string errorMessage, Action onFailure) =>
        BrainCloudManager.HandleFailure(errorMessage, onFailure);

    /// <summary>
    /// A quick method for ContentUIBehaviours to create a <see cref="FailureCallback"/> with <see cref="BrainCloudManager.HandleFailure(string, Action{ErrorResponse})"/>.
    /// </summary>
    protected FailureCallback OnFailure(string errorMessage, Action<ErrorResponse> onFailureER) =>
        BrainCloudManager.HandleFailure(errorMessage, onFailureER);

    /// <summary>
    /// A quick method for ContentUIBehaviours to create a <see cref="FailureCallback"/> with <see cref="BrainCloudManager.HandleFailure(string, Action{ErrorResponse, object})"/>.
    /// </summary>
    protected FailureCallback OnFailure(string errorMessage, Action<ErrorResponse, object> onFailureERO) =>
        BrainCloudManager.HandleFailure(errorMessage, onFailureERO);
